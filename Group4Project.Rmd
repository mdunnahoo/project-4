...
The exploratory plot above, plot A, shows the outcomes of matches for Manchester City (Man City) as the home team in the 2011-2012 season. Specifically, it includes all of Man City's home games leading up to the match that they played against Manchester United (Man United) as the away team on April 30, 2012. The outcomes of the matches are measured as a win (three points), loss (one point) or draw (zero points). Manchester City won the vast majority of those matches and had very few losses. It doesn't appear that Man City had any draws. The exploratory plot above, plot B, shows the outcomes of matches for Manchester United as the away team in the 2011-2012 season leading up to the team's match at Manchester City on April 30, 2012. Man United won most of those matches, it had some losses and very few draws. From these exploratory plots it seems reasonable to guess that Manchester City would win the match against Manchester United. 


## Prediction for Match

```{r, echo=FALSE, out.width='70%', fig.align="center"}
knitr::include_graphics("ManCityTable.png")
```

The article specifically investigates the Manchester City- Manchester United match on April 30, 2012. The paper creates a table based on all of the matches Manchester City played before its match against Manchester United (shown above and taken from the article by Jiri Lahvicka). 10,000,000 simulations were used to create the table and it predicts the result of the match in terms of the home team, Manchester City. The article predicts that Manchester City should likely win the match over 51% of the time. 
...
From our Monte Carlo simulations we also created a table in terms of the home team and we predicted that Manchester City should win the match against Manchester United over 49% of the time. The actual result of the game was that Manchester City did win 1-0. Our result differs slightly from the result in the article and this could be due to the randomness of sampling and our decreased simulation size. We ran 100,000 simulations instead of 10,000,000 because the simulations take a very long time to run. 

# Further Exploration

We ran simulations to predict the outcomes of certain matches. We chose to run simulations for three matches that resulted in either a home draw, win, or loss. Something we noticed is that our predictions gave an advantage to the home teams rather than the away teams. This makes sense because generally home teams tend to be more supported and well rested when games are played at their home stadiums. 
...
The exploratory plot above, plot A, shows the outcomes of matches for Chelsea as the home team in the 2014-2015 season. It shows the outcomes of Chelsea's home games leading up to the match that they played against Liverpool as the away team on May 10, 2015. Chelsea won the majority of those matches and had very few losses and even less draws. The exploratory plot above, plot B, shows the outcomes of matches for Liverpool as the away team in the 2014-2015 season leading up to the team's match at and against Chelsea. Liverpool won the majority of its matches, had some draws and even fewer losses. From these exploratory plots it is difficult to guess which team would have won the match, but Chelsea seems to be a strong guess because the team appears to have a few more wins than Liverpool. 
...
We ran simulations that modeled Chelsea as the home team and Liverpool as the away team that predicted the outcome of the May 5, 2015 match. We compiled our results into a table that models our predictions in terms of the home team (Chelsea). Our simulations found that Chelsea should win the match against Liverpool over fifty percent of the time and lose to Liverpool less than thirty percent of the time. Our findings predicted that Chelsea should have won the match, however the game resulted in a draw. 
...
We ran simulations that modeled Manchester City as the home team and Newcastle as the away team that predicted the outcome of the August 19, 2013 match. We compiled our results into a table that models our predictions in terms of the home team (Manchester City). Our simulations found that Man City should win the match against Newcastle over 60% of the time and lose to Newcastle less than 25% of the time. Our findings predicted that Man City should have won the match and our prediction was correct. 
...
We ran simulations that modeled Tottenham as the home team and Aston Villa as the away team that predicted the outcome of the April 11, 2015 match. We compiled our results into a table that models our predictions in terms of the home team (Tottenham). Our simulations found that Tottenham should win the match against Aston Villa over fifty percent of the time and lose to Aston Villa less than thirty percent of the time. Our findings predicted that the home team should have won the match, however Tottenham lost to Aston Villa. 
